...
//LRU算法函数
void Page_manager::LRU_run()
{
    vector<int> used(this->memory_size, -1); // 用于记录页面最近的使用情况
    for (int i = 0; i < this->page_access_sequence.size(); ++i) {
        bool found = false;
        time+=1;
        // 检查页面是否已经在内存中
        for (int k = 0; k < this->memory_size; ++k) {
            if (this->memory[k][0] == this->page_access_sequence[i]) {
                found = true;
                used[k] = i; // 更新页面的最近使用时间
                break;
            }
        }
        ...
        if (!found)
        {
            ...
            // 找到最久未使用的页面
            for (int k = 1; k < this->memory_size; ++k) {
                if (used[k] < minUsed) {
                    idx = k;
                    minUsed = used[k];
                }
            ...
            // 用新页面替换最久未使用的页面
            this->memory[idx][0] = this->page_access_sequence[i];
            used[idx] = i; // 更新页面的最近使用时间
            this->missing_page++; // 增加缺页计数，表示发生了页面错误

            // 打印当前内存状态
            cout << "当前内存状态：\n";
            for (int k = 0; k < this->memory_size; ++k) {
                if (this->memory[k][0] == 0) {
                    cout << "内存块 " << k + 1 << ":页面0 " << endl;
                } else {
                    cout << "内存块 " << k + 1 << ": 页面 " << this->memory[k][0] << endl;
                }
            }
            cout << "缺页次数：" << this->missing_page << endl;
        }
    }
    double missing=(double)this->missing_page/(double)this->time;
